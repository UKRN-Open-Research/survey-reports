...
format_priority_training <- function(priority, dat_training){
  ...
  training_opendata = filter(dat_training, OR_Area == "OpenData")
  ...
  priority_opendata = filter(priority, randomID %in% training_opendata$randomID, OR_Area == "OpenData")
  ...
  priority_training <- rbind(priority_openaccess, priority_opencode, priority_opendata, priority_preprints, priority_preregistration)
