...
# Change factor levels (eadership role)
dat$MajorLeadershipRole <- factor(dat$MajorLeadershipRole,
                                  levels = c("Yes",
                                             "No"))
...
p2 <- count(dat, MajorLeadershipRole) %>%
  ggplot(aes(x = MajorLeadershipRole, y = n, label = n)) +
...
p2_2 <- count(dat, MajorLeadershipRole) %>%
  ggplot(aes(x = MajorLeadershipRole, y = n, label = n)) +
  geom_bar(fill = "#13A89E", stat = "identity") +
  ylim(0, nrow(dat)) +
