...
priority_main$priority_count <- as.factor(priority_main$priority_count)
...
priority$Priority_OpenAccess <- sub("6|7|8", "6-8", priority$Priority_OpenAccess)
priority$Priority_OpenCode <- sub("6|7|8", "6-8", priority$Priority_OpenCode)
priority$Priority_OpenData <- sub("6|7|8", "6-8", priority$Priority_OpenData)
priority$Priority_Preprints <- sub("6|7|8", "6-8", priority$Priority_Preprints)
priority$Priority_Preregistration <- sub("6|7|8", "6-8", priority$Priority_Preregistration)

priority <- priority %>%
  pivot_longer(everything(), names_to = "OR_Area", values_to = "Rank")

priority$OR_Area <- sub("Priority_", "", priority$OR_Area)

# Factor levels
priority$Rank <- factor(priority$Rank,
                        levels = c("6-8",
                                   "5",
                                   "4",
                                   "3",
...
priority %>% group_by(OR_Area) %>% count(Rank) %>%
  ...
  xlab("Open Research Areas") + ylab("Number of Respondents") +
  theme(plot.title = element_text(hjust = 0.5)) +
